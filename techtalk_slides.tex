...
\begin{frame}{Best strategy}
    %  TODO: these slides are too wordy, simplify
	\begin{itemize}
		% \item There are rules for regular morphology
		\item Irregulars are arbitrary
		\begin{itemize}
			\item Which verbs?
			\item How do they conjugate?
		\end{itemize}
		% \item Rules might be tough to derive
		\item Machine Learning may work
		\begin{itemize}
			\item prediction or fitting?	%by which I mean are we trying to predict irregulars or simply capture the ones we know about
		\end{itemize}
	\end{itemize}
\end{frame}
...
\begin{frame}{Finite problem set}
	\begin{itemize}
		...
		\item Prediction is less important
	\end{itemize}
\end{frame}
...
\begin{frame}{Tracking saliency}
	\begin{itemize}
		\item An advanced NLG system should track saliency for pronoun use
		\item Syntactic structure strongly influences saliency \pause
		\item Pronoun distribution is based on known principles
		\item The AI system should also share those principles
	\end{itemize}
\end{frame}
